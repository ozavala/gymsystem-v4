...
def create_member
  @gym.members.create(
    ...
    zip: ' 091650',
    phone: Faker::PhoneNumber.cell_phone,
    birthday: Time.now - 25.years + rand(0..5).years + rand(1..30).days,
    membership_type: ["Cortesía", "Semanal","Mensual", "Anual", "Visita"].sample,
    source: [":Facebook", "Instagram", "Presencial","Sitio_Web", "Referencial"].sample,
    level: ["Visitante", "Principiante", "Intermedio", "Avanzado", "Profesional"].sample,
    from_date: Time.now-20.days+rand(0..19).days,
    membership_price: [65.00, 10.00, 350.00, 700.00].sample,
    is_active: [true, false].sample
        )
end

20.times do create_member; end
